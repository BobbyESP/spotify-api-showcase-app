package com.bobbyesp.spotifyapishowcaseapp.util.pagination
...
inline fun <T : Any> createPager(
    ...
    return try {
        if (isLogged) {
            if (clientApi == null || clientApi.token.shouldRefresh()) throw TokenRefreshException("Client API is null; checked on createPager function") // Strange error, should never happen when app is ready for the public.
